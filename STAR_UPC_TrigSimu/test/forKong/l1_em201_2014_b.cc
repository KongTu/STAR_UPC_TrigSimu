...
void l1_em201_2014_b(Board& em201)
{
  // INPUT:
  ...
  // ACTION:
  const int R0 = em201.registers[0];
  const int R1 = em201.registers[1];
  ...
  for (int ichn = 0; ichn < 6; ++ichn) {
    bemcHT |= em201.channels[ichn] >> 10 & 0x3f;

    bemcTPBit[ichn] = em201.channels[ichn] >> 8 & 0x1;
    bemcTP |= bemcTPBit[ichn];

    bemcHTTPBit[ichn] = em201.channels[ichn] >> 9 & 0x1;
    bemcHTTP |= bemcHTTPBit[ichn];

    bemcHTUPCBit[ichn] = em201.channels[ichn] >> 15 & 0x1;

    counterDAQ10K += em201.channels[ichn] >> 7 & 0x1;
  }
  ...
  for (int ichn = 6; ichn < 8; ++ichn) {
    eemcHT |= em201.channels[ichn] >> 14 & 0x3;
  }
