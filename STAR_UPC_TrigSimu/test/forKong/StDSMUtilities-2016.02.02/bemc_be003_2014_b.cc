...
void bemc_be003_2014_b(Board& bw003)
{
  // INPUT:
  ...
  for(int ichn = 0; ichn < 10; ichn++){
    unsigned int ht = bw003.channels[ichn] & 0x3f;
    for(int ireg = 0; ireg < 6; ireg++){
      highTowerBits[ichn][ireg] = ht > bw003.registers[ireg];
    }
    unsigned int tp = bw003.channels[ichn] >> 6 & 0x3f;
    trigPatchBits[ichn] = tp > bw003.registers[5];
  }
