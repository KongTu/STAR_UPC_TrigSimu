...
inline void swapL1_DSM_Data(L1_DSM_Data& L1data)
{
  ...
  transform(L1data.TPCMask,L1data.TPCMask+8,L1data.TPCMask,bswap16);
...
inline void swapQTBlock(QTBlock& qt)
{
  ...
  transform(qt.data,qt.data+qt.length/4,qt.data,bswap32);
}
  ...
  case BC1_CONF_NUM:
    {
      ...
      if (strncmp(bc1.name,"BC1",3) == 0) swapBELayerBlock(bc1);
      break;
    }
  case MIX_CONF_NUM:
    {
      MIXBlock& mix = (MIXBlock&)data;
      if (strncmp(mix.name,"MIX",3) == 0) swapMIXBlock(mix);
      break;
    }
  case BCW_CONF_NUM:
    {
      BWestBlock& bcw = (BWestBlock&)data;
      if (strncmp(bcw.name,"BCW",3) == 0) swapBWestBlock(bcw);
      break;
    }
  case BCE_CONF_NUM:
    {
      BEastBlock& bce = (BEastBlock&)data;
      if (strncmp(bce.name,"BCE",3) == 0) swapBEastBlock(bce);
      break;
    }
  case BBC_CONF_NUM:
    {
      BBCBlock& bbc = (BBCBlock&)data;
      if (strncmp(bbc.name,"BBC",3) == 0) swapBBCBlock(bbc);
      break;
    }
  case FMS_CONF_NUM:
    {
      FMSBlock& fms = (FMSBlock&)data;
      if (strncmp(fms.name,"FMS",3) == 0) swapFMSBlock(fms);
      break;
    }
  case MXQ_CONF_NUM:
    {
      QTBlock& mxq = (QTBlock&)data;
      if (strncmp(mxq.name,"MXQ",3) == 0) swapQTBlock(mxq);
      break;      
    }
  case FEQ_CONF_NUM:
    {
      QTBlock& feq = (QTBlock&)data;
      if (strncmp(feq.name,"FEQ",3) == 0) swapQTBlock(feq);
      break;
    }
  case BBQ_CONF_NUM:
    {
      QTBlock& bbq = (QTBlock&)data;
      if (strncmp(bbq.name,"BBQ",3) == 0) swapQTBlock(bbq);
      break;      
    }
  case QT1_CONF_NUM:
    {
      QTBlock& qt1 = (QTBlock&)data;
      if (strncmp(qt1.name,"QT1",3) == 0) swapQTBlock(qt1);
      else printf("%s:%d: bad QT1 header\n",__FILE__,__LINE__);
      break;      
    }
  case QT2_CONF_NUM:
    {
      QTBlock& qt2 = (QTBlock&)data;
      if (strncmp(qt2.name,"QT2",3) == 0) swapQTBlock(qt2);
      else printf("%s:%d: bad QT2 header\n",__FILE__,__LINE__);
      break;      
    }
  case QT3_CONF_NUM:
    {
      QTBlock& qt3 = (QTBlock&)data;
      if (strncmp(qt3.name,"QT3",3) == 0) swapQTBlock(qt3);
      else printf("%s:%d: bad QT3 header\n",__FILE__,__LINE__);
      break;      
    }
  case QT4_CONF_NUM:
    {
      QTBlock& qt4 = (QTBlock&)data;
      if (strncmp(qt4.name,"QT4",3) == 0) swapQTBlock(qt4);
      else printf("%s:%d: bad QT4 header\n",__FILE__,__LINE__);
      break;
    }
  default:
    {
...
inline void swapTriggerDataBlk(TriggerDataBlk& trgData)
{
  ...
  EvtDescData* evtDesc = (EvtDescData*)((char*)&trgData+trgData.EventDesc_ofl.offset);
  ...
  L1_DSM_Data* L1data = (L1_DSM_Data*)((char*)&trgData+trgData.L1_DSM_ofl.offset);
  ...
  TrgSumData* trgSum = (TrgSumData*)((char*)&trgData+trgData.Summary_ofl.offset);
  ...
  for (int crate = 0; crate < MAX_OFFLEN; ++crate) {
    ...
    if (trgData.MainX[crate].offset && trgData.MainX[crate].length) {
      DataBlock* data = (DataBlock*)((char*)&trgData+trgData.MainX[crate].offset);
  ...
  for (int i = 0; i < nentries; ++i) {
    ...
    TrgOfflen* PrePostX = (TrgOfflen*)((char*)&trgData+trgData.PrePostList[i]);
    for (int crate = 0; crate < MAX_OFFLEN; ++crate) {
      swapTrgOfflen(PrePostX[crate]);
      if (PrePostX[crate].offset && PrePostX[crate].length) {
	DataBlock* data = (DataBlock*)((char*)&trgData+PrePostX[crate].offset);
	swapDataBlockAtCrate(*data, crate);
      }
    }
  }
...
inline void decodeQT(const QTBlock& qt, int crate, unsigned short adc[16][32], unsigned short tdc[16][32])
{
  int sz = qt.length/4;
  ...
  for (int i = 0; i < sz-1;) {
    ...
    while (nlines--) {
      ...
      assert(0 <= ch && ch < 32);
      adc[addr][ch] = d & 0xfff;
      //tdc[addr][ch] = d >> 12 & 0x1f;
      tdc[addr][ch] = d >> 16 & 0x1f;
    }
  }
}
