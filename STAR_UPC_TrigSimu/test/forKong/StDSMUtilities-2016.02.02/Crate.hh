...
//DAQ10k Implementation --zchang
const int DQ301_BASE_ADDRESS = 0x15000000;
...
const int TF001_BASE_ADDRESS = 0x14000000;
const int TF002_BASE_ADDRESS = 0x15000000;
const int TF003_BASE_ADDRESS = 0x17000000;
const int TF004_BASE_ADDRESS = 0x18000000;
const int TF005_BASE_ADDRESS = 0x1a000000;
const int TF006_BASE_ADDRESS = 0x1b000000;

const int EM202_BASE_ADDRESS = 0x1d000000;
const int MT102_BASE_ADDRESS = 0x1e000000;
const int TF202_BASE_ADDRESS = 0x1c000000;
...
const int PX001_BASE_ADDRESS = 0x14000000;
const int VP003_BASE_ADDRESS = 0x16000000;
const int VP004_BASE_ADDRESS = 0x18000000;
const int MT002_BASE_ADDRESS = 0x19000000;
const int MT003_BASE_ADDRESS = 0x1a000000;
const int MT004_BASE_ADDRESS = 0x1b000000;
const int MT005_BASE_ADDRESS = 0x1c000000;
const int MT006_BASE_ADDRESS = 0x1d000000;
const int MT007_BASE_ADDRESS = 0x1e000000;
const int MT008_BASE_ADDRESS = 0x1f000000;
...
struct Crate {
  ...
  void read(const TriggerDataBlk& event, int crateid);
  void read(const TriggerDataBlk& event, int crateid, int prepost);
...
inline void Crate::read(const TriggerDataBlk& event, int crateid)
{
  clear();
  switch (crateid) {
    // DSM crates
  case L1_CONF_NUM:
    if (L1_DSM_Data* L1data = (L1_DSM_Data*)((char*)&event+event.L1_DSM_ofl.offset)) {
      ...
      copy_and_swap(boardAt(DQ301_BASE_ADDRESS).channels,L1data->TPCMask);
  ...
  case MIX_CONF_NUM:
    if (event.MainX[MIX_CONF_NUM].offset && event.MainX[MIX_CONF_NUM].length) {
      ...
      copy_and_swap(boardAt(PP101_BASE_ADDRESS).channels,mix->FPDEastNSLayer1);
      //copy_and_swap((unsigned char*)boardAt(MT101_BASE_ADDRESS).channels,mix->MTD_P2PLayer1);
      // Run 13
      unsigned char buffer[16];
      copy_and_swap(buffer,mix->MTD_P2PLayer1); unpack(boardAt(MT101_BASE_ADDRESS).channels,buffer);
      ...
      //DAQ10k -- zchang Run14
      copy_and_swap(boardAt(EM202_BASE_ADDRESS).channels,mix->TPCpreMask);
      copy_and_swap(boardAt(MT102_BASE_ADDRESS).channels,mix->TPCpreMask+8);
      copy_and_swap(boardAt(TF202_BASE_ADDRESS).channels,mix->TPCpreMask+16);
    }
    break;
  case BCW_CONF_NUM:
  ...
  case FMS_CONF_NUM:
    if (event.MainX[FMS_CONF_NUM].offset && event.MainX[FMS_CONF_NUM].length) {
      ...
      copy_and_swap((unsigned char*)boardAt(FM101_BASE_ADDRESS).channels,fms->FMS+32);
      copy_and_swap((unsigned char*)boardAt(FM003_BASE_ADDRESS).channels,fms->FMS+48);
      copy_and_swap((unsigned char*)boardAt(FM004_BASE_ADDRESS).channels,fms->FMS+64);
      copy_and_swap((unsigned char*)boardAt(FM102_BASE_ADDRESS).channels,fms->FMS+80);
      copy_and_swap((unsigned char*)boardAt(FM005_BASE_ADDRESS).channels,fms->FMS+96);
      copy_and_swap((unsigned char*)boardAt(FM006_BASE_ADDRESS).channels,fms->FMS+112);
      copy_and_swap((unsigned char*)boardAt(FM007_BASE_ADDRESS).channels,fms->FMS+128);
      copy_and_swap((unsigned char*)boardAt(FM008_BASE_ADDRESS).channels,fms->FMS+144);
      copy_and_swap((unsigned char*)boardAt(FM103_BASE_ADDRESS).channels,fms->FMS+160);
      copy_and_swap((unsigned char*)boardAt(FM009_BASE_ADDRESS).channels,fms->FMS+176);
      copy_and_swap((unsigned char*)boardAt(FM010_BASE_ADDRESS).channels,fms->FMS+192);
      copy_and_swap((unsigned char*)boardAt(FM011_BASE_ADDRESS).channels,fms->FMS+208);
      copy_and_swap((unsigned char*)boardAt(FM012_BASE_ADDRESS).channels,fms->FMS+224);
      copy_and_swap((unsigned char*)boardAt(FM104_BASE_ADDRESS).channels,fms->FMS+240);
    }
    break;
    // QT crates
  case MXQ_CONF_NUM:
  ...
  case QT3_CONF_NUM:
  case QT4_CONF_NUM:
    if (event.MainX[crateid].offset && event.MainX[crateid].length) {
      QTBlock* qt = (QTBlock*)((char*)&event+event.MainX[crateid].offset);
      if (strncmp(qt->name,crateNames[crateid],3) == 0) decodeQT(*qt,crateid);
      else printf("%s:%d: bad %s header\n",__FILE__,__LINE__,crateNames[crateid]);
    }
    break;
  default:
    break;
...
//depreciated
#if 0
inline void Crate::read(const TriggerDataBlk& event, int crateid, int prepost)
{
  clear();
  ...
  assert(prepost >= 0 && prepost < evtDesc->npre+evtDesc->npost);
  TrgOfflen* PrePostX = (TrgOfflen*)((char*)&event+event.PrePostList[prepost]);

  switch (crateid) {
    // DSM crates
  case L1_CONF_NUM:
    //
    // No pre/post data saved for L1 crate
    // See http://www.star.bnl.gov/HyperNews-star/protected/get/startrig/4341/2.html
    //
    break;
  case BC1_CONF_NUM:
    if (PrePostX[BC1_CONF_NUM].offset && PrePostX[BC1_CONF_NUM].length) {
      BELayerBlock* bc1 = (BELayerBlock*)((char*)&event+PrePostX[BC1_CONF_NUM].offset);
      assert(strncmp(bc1->name,"BC1",3) == 0);
      copy_and_swap(boardAt(BC101_BASE_ADDRESS).channels,bc1->BEMClayer1);
      copy_and_swap(boardAt(BC102_BASE_ADDRESS).channels,bc1->BEMClayer1+8);
      copy_and_swap(boardAt(BC103_BASE_ADDRESS).channels,bc1->BEMClayer1+16);
    ...
    break;
  case MIX_CONF_NUM:
    if (PrePostX[MIX_CONF_NUM].offset && PrePostX[MIX_CONF_NUM].length) {
      MIXBlock* mix = (MIXBlock*)((char*)&event+PrePostX[MIX_CONF_NUM].offset);
      assert(strncmp(mix->name,"MIX",3) == 0);
//      copy_and_swap(boardAt(FE101_BASE_ADDRESS).channels,mix->FPDEastNSLayer1);
      copy_and_swap(boardAt(PP101_BASE_ADDRESS).channels,mix->FPDEastNSLayer1);
      //zchang
    ...
    break;
  case BCW_CONF_NUM:
    if (PrePostX[BCW_CONF_NUM].offset && PrePostX[BCW_CONF_NUM].length) {
      BWestBlock* bcw = (BWestBlock*)((char*)&event+PrePostX[BCW_CONF_NUM].offset);
      assert(strncmp(bcw->name,"BCW",3) == 0);
      unsigned char buffer[16];
      copy_and_swap(buffer,bcw->BEMCWest); unpack(boardAt(BW001_BASE_ADDRESS).channels,buffer);
      copy_and_swap(buffer,bcw->BEMCWest+16); unpack(boardAt(BW002_BASE_ADDRESS).channels,buffer);
    ...
    break;
  case BCE_CONF_NUM:
    if (PrePostX[BCE_CONF_NUM].offset && PrePostX[BCE_CONF_NUM].length) {
      BEastBlock* bce = (BEastBlock*)((char*)&event+PrePostX[BCE_CONF_NUM].offset);
      assert(strncmp(bce->name,"BCE",3) == 0);
      unsigned char buffer[16];
      copy_and_swap(buffer,bce->BEMCEast); unpack(boardAt(BE001_BASE_ADDRESS).channels,buffer);
      copy_and_swap(buffer,bce->BEMCEast+16); unpack(boardAt(BE002_BASE_ADDRESS).channels,buffer);
    ...
    break;
  case BBC_CONF_NUM:
    if (PrePostX[BBC_CONF_NUM].offset && PrePostX[BBC_CONF_NUM].length) {
      BBCBlock* bbc = (BBCBlock*)((char*)&event+PrePostX[BBC_CONF_NUM].offset);
      assert(strncmp(bbc->name,"BBC",3) == 0);
      copy_and_swap(boardAt(BB101_BASE_ADDRESS).channels,bbc->BBClayer1);
      copy_and_swap(boardAt(BB102_BASE_ADDRESS).channels,bbc->BBClayer1+8);
      copy_and_swap(boardAt(ZD101_BASE_ADDRESS).channels,bbc->ZDClayer1);
    ...
    break;
  case FMS_CONF_NUM:
    if (PrePostX[FMS_CONF_NUM].offset && PrePostX[FMS_CONF_NUM].length) {
      FMSBlock* fms = (FMSBlock*)((char*)&event+PrePostX[FMS_CONF_NUM].offset);
      assert(strncmp(fms->name,"FMS",3) == 0);
      copy_and_swap((unsigned char*)boardAt(FM001_BASE_ADDRESS).channels,fms->FMS);
      copy_and_swap((unsigned char*)boardAt(FM002_BASE_ADDRESS).channels,fms->FMS+16);
      copy_and_swap((unsigned char*)boardAt(FM101_BASE_ADDRESS).channels,fms->FMS+32);
  ...
  case QT3_CONF_NUM:
  case QT4_CONF_NUM:
    if (PrePostX[crateid].offset && PrePostX[crateid].length) {
      QTBlock* qt = (QTBlock*)((char*)&event+PrePostX[crateid].offset);
      assert(strncmp(qt->name,crateNames[crateid],3) == 0);
      decodeQT(*qt,crateid);
    }
    break;
  default:
...
{
  char name[5];
  strncpy(name,qt.name,4);
  name[4] = 0;
  int sz = qt.length/4;
  //assert(qt.data[sz-1] == 0xac10);
  if (qt.data[sz-1] != 0xac10) {
    printf("decodeQT [name=%s crate=%d]: bad footer 0x%08x\n",name,crate,qt.data[sz-1]);
    return;
  }
  for (int i = 0; i < sz-1;) {
    // Header
    ...
    if (crate != (h >> 24 & 0xff)) {
      printf("decodeQT [name=%s crate=%d]: bad header 0x%08x\n",name,crate,h);
      return;
    }
    int nlines = h & 0xff;
    int addr = h >> 16 & 0xff;
    ...
    if (addr < 0x10 || addr >= 0x20) {
      printf("decodeQT [name=%s crate=%d]: bad address 0x%08x\n",name,crate,addr);
      return;
    }
    addr -= 0x10;
    // Data
    while (nlines--) {
      ...
      //assert(0 <= ch && ch < 32);
      if (ch < 0 || ch >= 32) {
	printf("decodeQT [name=%s crate=%d]: bad channel %d\n",name,crate,ch);
	return;
      }
